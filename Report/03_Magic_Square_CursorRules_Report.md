# MagicSquare .cursorrules 작성 보고서 (갱신본)

## 개요
- 목적: MagicSquare 프로젝트에 맞는 Cursor 규칙 파일을 정의하여 TDD/ECB 기반 개발 일관성을 확보
- 적용 위치:
  - 프로젝트 루트: `.cursorrules`
  - 소스 규칙 경로: `src/.cursor/rules/.cursorrules`
- 작성 기준: 사용자 지정 섹션, 형식 규칙(80자 구분선/2칸 들여쓰기), 한국어 설명 반영
...
## 핵심 설계 요약
- 코드 스타일: Python 3.10+, PEP8 엄격 준수, 타입힌트/Google docstring 의무화
- 아키텍처: ECB 3계층(`boundary`, `control`, `entity`) 역할 및 의존성 방향 고정
- TDD 규칙: `red_phase`, `green_phase`, `refactor_phase`를 `description/rules/must_not` 구조로 통일
- 테스트 정책: pytest, AAA, 최소 커버리지 80%, fixture 기본 scope 함수 단위
- 금지 규칙: `print()`, 하드코딩 상수, `except` 단독, boundary에서 entity 직접 import 금지
- 파일 구조: `src`/`tests` 기준 레이어별 폴더 예시 제공
- AI 행동 규칙: 코드 생성 전/중/후 점검 항목과 TDD 위반 경고 조건 명시

## 최종 `src/.cursor/rules/.cursorrules` 내용
